...
const StudentModel = require("../Models/studenet.model");
...
studentRoute.patch("/:studentID/update", async (req, res) => {
  ...
  if (student.userId === userId) {
    const updated_student = await StudentModel.findOneAndUpdate(
      { _id: studentId },
      req.body,
      { new: true }
    );
...
studentRoute.delete("/:studentID/delete", async (req, res) => {
  ...
  if (student.userId === userId) {
    ...
    await TestsModel.deleteMany({ studentId: studentId });
    return res.send({
